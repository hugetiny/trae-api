        ...
        async with AsyncClient() as client:
            try:
                async with httpx_sse.aconnect_sse(
                    ...
                    # httpx_sse.aconnect_sse returns an EventSource object, which wraps the response.
                    # The actual response is available as response.response
                    if response.response.status_code != 200:
                         print(f"Upstream Error: {response.response.status_code}")
                         yield f"data: {json.dumps({'error': f'Upstream {response.response.status_code}'})}\n\n"
                         return
                ...
                print(f"Exception in stream_response: {e}")
